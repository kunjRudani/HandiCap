...
class Tp1 extends StatefulWidget {
  const Tp1({Key? key}) : super(key: key);

  @override
  State<Tp1> createState() => _TpState();
}

class _TpState extends State<Tp1> {
  List _allresult=[];
  List _resultlist=[];
  ...
  getClientStream() async {
    var data = await FirebaseFirestore.instance.collection("HandicapeOfWriter").orderBy("city").get();
    setState(() {
            ...
            Expanded(
              child: Container(
                ...
                child: StreamBuilder<QuerySnapshot>(
                    stream:  FirebaseFirestore.instance.collection("HandicapeOfWriter").snapshots(),
                    builder: (context, snapshot) {
                      return ListView.builder(
                        itemCount: _resultlist.length,
                        itemBuilder: (context, index) {
                          DocumentSnapshot HandicapeOfWriter = (snapshot.data! as QuerySnapshot).docs[index];
                          return InkWell(
                            onTap: (){
